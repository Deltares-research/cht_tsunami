...
def default_tfluct(self):
    ...
    if self.tfluct_solver:
        raise Exception(
            "You set solver.tfluct_solver=True, but solver.tfluct has not been set."
        )
    pass
...
class SharpClawSolver(Solver):
    ...
    def setup(self, solution):
        ...
        if (
            self.lim_type == 2
            and self.weno_order != 5
            and self.kernel_language == "Python"
        ):
            raise Exception(
                "Only 5th-order WENO reconstruction is implemented in Python kernels. \
        ...
        if self.kernel_language == "Fortran":
            ...
            self._set_fortran_parameters(
                state, self.fmod.clawparams, self.fmod.workspace, self.fmod.reconstruct
            )
    ...
    def __del__(self):
        ...
        if self.kernel_language == "Fortran":
            ...
            self.fmod.reconstruct.dealloc_recon_workspace(
                self.fmod.clawparams.lim_type, self.fmod.clawparams.char_decomp
            )
            del self.fmod
    ...
    def step(self, solution, take_one_step, tstart, tend):
        ...
        if self.time_integrator == "Euler":
            ...
            self._registers[0].q = 0.75 * state.q + 0.25 * (
                self._registers[0].q + self.dq(self._registers[0])
            )
            self._registers[0].t = state.t + 0.5 * self.dt

            if self.call_before_step_each_stage:
                self.before_step(self, self._registers[0])
            state.q = 1.0 / 3.0 * state.q + 2.0 / 3.0 * (
                self._registers[0].q + self.dq(self._registers[0])
            )
        ...
        ### Linear multistep methods ###
            ...
            if step_index < num_steps:
                ...
                if self.time_integrator == "SSPLMMk2":
                    ...
                    state.q = (
                        ...
                        + delta * self._registers[-num_steps].q
                    )
                else:
                    omega_k_minus_1 = sum(self.prev_dt_values[1:]) / self.dt
                    ...
                    state.q = (
                        beta_k_minus_1
                        * (r * state.q + self.dt * self.prev_dq_dt_values[-1])
            ...
            if step_index < len(self._registers):
                ...
                state.q = (
                    self.alpha[-1] * self._registers[-1].q
                    + self.beta[-1] * self.dt * self.prev_dq_dt_values[-1]
                )
                for i in range(self.lmm_steps - 1):
                    state.q += (
                        self.alpha[i] * self._registers[i].q
                        + self.beta[i] * self.dt * self.prev_dq_dt_values[i]
                    )
    ...
    def ssp104(self, state):
        ...
        s1.q = state.q + self.dt * self.dq_dt / 6.0
        s1.t = state.t + self.dt / 6.0

        for i in range(4):
            if self.call_before_step_each_stage:
                self.before_step(self, s1)
            s1.q = s1.q + self.dq(s1) / 6.0
            s1.t = s1.t + self.dt / 6.0

        state.q = state.q / 25.0 + 0.36 * s1.q
        s1.q = 15.0 * state.q - 5.0 * s1.q
        s1.t = state.t + self.dt / 3.0
    ...
    def update_saved_values(self, state, step_index):
        ...
        if self.prev_dt_values == []:
            ...
            self.prev_dt_values.append(0.0)  # Not used
            if "SSPLMM" in self.time_integrator:
                cfl = self.cfl.get_cached_max()
            ...
            if "SSPLMM" in self.time_integrator:
                ...
                if step_index <= len(self._registers):  # Startup
                    ...
                    self.prev_dt_values = (
                        self.prev_dt_values[1:] + self.prev_dt_values[:1]
                    )
                    self.prev_dt_values[-1] = self.dt_old
                    self.prev_dtFE_values = (
                        self.prev_dtFE_values[1:] + self.prev_dtFE_values[:1]
                    )
    ...
    def _set_fortran_parameters(self, state, clawparams, workspace, reconstruct):
        ...
        workspace.alloc_workspace(
            maxnx, self.num_ghost, state.num_eqn, self.num_waves, self.char_decomp
        )
        reconstruct.alloc_recon_workspace(
            maxnx,
            self.num_ghost,
            state.num_eqn,
            self.num_waves,
            clawparams.lim_type,
            clawparams.char_decomp,
        )
    ...
    def _allocate_registers(self, solution):
        ...
        if self.time_integrator == "Euler":
            ...
            nregisters = len(self.b) + 1
        elif self.time_integrator == "SSPLMMk2":
            nregisters = self.lmm_steps
        elif self.time_integrator == "SSPLMMk3":
            nregisters = self.lmm_steps
        elif self.time_integrator == "LMM":
    ...
    def get_dt_new(self):
        ...
        if "SSPLMM" in self.time_integrator:
            ...
            if step_index < len(self._registers):
                ...
                if (
                    self.time_integrator == "SSPLMMk3"
                    and self.check_lmm_cond
                    and not self.lmm_cond
                ):
                    rho = 0.6 if len(self._registers) == 4 else 0.57
                    self.dt = rho * self.dt
            else:
...
class SharpClawSolver2D(SharpClawSolver):
    ...
    def dq_hyperbolic(self, state):
        ...
        if self.kernel_language == "Fortran":
            ...
            dq, cfl = self.fmod.flux2(
                q, self.auxbc, self.dt, state.t, num_ghost, maxm, mx, my, rpn2, tfluct2
            )

        else:
            raise Exception("Only Fortran kernels are supported in 2D.")
...
class SharpClawSolver3D(SharpClawSolver):
    ...
    def dq_hyperbolic(self, state):
        ...
        if self.kernel_language == "Fortran":
            ...
            raise Exception("Only Fortran kernels are supported in 3D.")
