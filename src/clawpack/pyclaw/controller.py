...
from .util import LOGGING_LEVELS, FrameCounter
...
class Controller(object):
    ...
    def __init__(self):
        ...
        ``default = 10``"""
        self.out_times = np.linspace(
            0.0, self.tfinal, self.num_output_times - self.start_frame
        )  # Outstyle 2
    ...
    def check_validity(self):
        ...
        if not valid:
            raise Exception(
                "The solver failed to initialize properly because " + reason
            )
    ...
    def load_frame(self, frame_number):
        try:
            ...
            print(
                "Cannot plot frame %s; only %s frames available"
    ...
    def run(self):
        ...
        if self.output_style == 1:
            output_times = np.linspace(
                self.solution.t, self.tfinal, self.num_output_times + 1
            )
        elif self.output_style == 2:
            output_times = self.out_times
        elif self.output_style == 3:
            output_times = np.ones((self.num_output_times + 1 - self.start_frame))
        else:
        ...
        self.log_info("Solution %s computed for time t=%f" % (frame, self.solution.t))
        ...
        for t in output_times[1:]:
            ...
            self.log_info(
                "Solution %s computed for time t=%f" % (frame, self.solution.t)
            )
...
class OutputController(object):
    ...
    def file_format(self, value):
        if value.lower() == "petsc":
            self._io_module = __import__(
                "clawpack.petclaw.fileio.petsc", fromlist=["clawpack.petclaw.fileio"]
            )
            self._file_format = value
        elif value.lower()[:6] == "binary":
            # could be 'binary64' or 'binary32'
            self._io_module = __import__(
                "clawpack.pyclaw.fileio.binary", fromlist=["clawpack.pyclaw.fileio"]
            )
            self._file_format = value
        else:
            self._io_module = __import__(
                "clawpack.pyclaw.fileio.%s" % value, fromlist=["clawpack.pyclaw.fileio"]
            )
            self._file_format = value
    ...
    def get_time(self, frame_num):
        ...
        if "read_t" in dir(self._io_module):
            return self._io_module.read_t(
                frame_num, path=self.output_path, file_prefix=self.file_prefix
            )[0]
        else:
            return None
