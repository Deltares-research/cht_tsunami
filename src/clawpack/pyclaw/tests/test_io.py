...
class IOTest:
    ...
    def test_io_from_hdf5_with_aux(self):
        ...
        self.read_write_and_compare(
            self.file_formats, regression_dir, "hdf5", 0, aux=True
        )

    def read_write_and_compare(
        self, file_formats, regression_dir, regression_format, frame_num, aux=False
    ):
        ...
        ref_sol.read(
            frame_num, path=regression_dir, file_format=regression_format, read_aux=aux
        )
        if aux:
