    ...
    for mw in range(wave.shape[1]):
        ...
        if limit_func is not None:
            for m in range(num_eqn):
                cfl = np.abs(
                    s[mw, 1:-1]
                    * (dtdx[1:-2] * spos[mw, :] + (1 - spos[mw, :]) * dtdx[2:-1])
                )
...
def hyperbee_limiter(r, cfl):
    ...
    return np.choose(
        ...
        [
            0.0,
            1.0,
            2.0 * rdur * (cfl * rmin + 1.0 - r**cfl) / (cfmod1 * (1.0 - cfmod2) * rmin),
        ],
    )
...
def superpower_limiter(r, cfl, caut=1.0):
    ...
    pp = ((r <= 1.0) * np.abs(2.0 / cfl) * caut) * 2.0 * s3 + (r > 1.0) * (
        np.abs(2.0) / (1.0 - cfl) * caut
    ) * 2.0 * s2
