...
def weno5_wave(q, wave, s):
    ...
    for m1 in [1, 2]:
        ...
        for mw in range(num_waves):
            ...
            z = (s1 * (t2 - t1) + (0.5 * s3 - 0.25) * (t3 - t2)) / 3.0 + im * (
                theta2 + 6.0 * wnorm2 - theta3
            ) / 12.0
            u = np.where(wnorm2 > 1.0e-14, z, 0.0)
            wnorm2 = np.where(wnorm2 > 1.0e-14, 1.0 / wnorm2, 1.0)

            for m in range(num_eqn):
                if m1 == 1:
                    qr[m, LL:UL] += u * wave[m, mw, LL:UL] * wnorm2
                else:
                    ql[m, LL + 1 : UL + 1] += u * wave[m, mw, LL:UL] * wnorm2
