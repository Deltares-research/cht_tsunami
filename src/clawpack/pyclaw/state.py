...
class State(object):
    ...
    def set_cparam(self, fortran_module):
        ...
        if hasattr(fortran_module, "cparam"):
            try:
                paramlist = [
                    parm for parm in fortran_module.cparam.__dir__() if "__" not in parm
                ]
            ...
            if not set(paramlist) <= set(self.problem_data.keys()):
                raise Exception("""Some required value(s) in the cparam common 
                                   ...
                                   set in problem_data.""")
            for global_var_name, global_var_value in self.problem_data.items():
    ...
    def get_qbc_from_q(self, num_ghost, qbc):
        ...
        if num_dim == 1:
            ...
            qbc[:, num_ghost:-num_ghost, num_ghost:-num_ghost, num_ghost:-num_ghost] = (
                self.q
            )
        else:
            raise Exception("Assumption (1 <= num_dim <= 3) violated.")
