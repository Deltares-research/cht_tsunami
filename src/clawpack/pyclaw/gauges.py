...
class GaugeSolution(object):
    ...
    def read(self, gauge_id, path=None, use_pandas=False):
        ...
        if self.id != gauge_id:
            raise ValueError("Gauge ID requested does not match ID inside ", "file!")
        ...
        if file_format[:6] == "binary":
            ...
            if not os.path.isfile(gauge_path):
                msg = (
                    "No data in .txt file and did not find "
        ...
        if num_eqn != self.q.shape[0]:
            raise ValueError(
                "Number of fields in gauge file does not match",
        ...
        if len(self.location) == 2:
            ...
            if self.gtype == "lagrangian":
                ...
                self.particle_path = numpy.vstack(
                    (self.t, self.q[1, :], self.q[2, :])
                ).T
            else:
                # set the lagrangian path to a single fixed location at t=t0:
                self.particle_path = numpy.array(
                    [[self.t[0], self.location[0], self.location[1]]]
                )
    ...
    def write(self, path=None, format="%+.15e"):
        ...
        with open(os.path.join(path, gauge_file_name), "w") as gauge_file:
            gauge_file.write(
                "# gauge_id= %s location=( %s %s ) num_eqn= %s\n"
...
def compare_gauges(paths, gauge_id, fields="all"):
    ...
    for i, n in enumerate(fields):
        ...
        axes.plot(gauges[0].t, numpy.abs(gauges[0].q[n, :] - gauges[1].q[n, :]), "r")
        axes.set_xlabel("t")
...
def compare_old_gauges(
    old_path, new_path, gauge_id, plot=False, abs_tol=1e-14, rel_tol=0.0, verbose=False
):
    ...
    if verbose:
        ...
        print(r"  arg(||\Delta q||_\infty = ", numpy.argmax(q - gauge.q.transpose()))
