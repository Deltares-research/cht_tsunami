...
identity_map = {"1": identity_map_1d, "2": identity_map_2d, "3": identity_map_3d}
    ...
    **Mappings:**
    ...
    def add_dimension(self, dimension):
        ...
        if dimension.name in self._dimensions:
            raise Exception(
                "Unable to add dimension. A dimension"
                + " of the same name: {name}, already exists.".format(
                    name=dimension.name
                )
            )
    ...
    def _compute_c_centers(self, recompute=False):
        ...
        if (
            recompute
            or (self._c_centers is None)
            or any([c is None for c in self.get_dim_attribute("_centers")])
        ):
    ...
    def _compute_c_nodes(self, recompute=False):
        ...
        if (
            recompute
            or (self._c_nodes is None)
            or any([c is None for c in self.get_dim_attribute("_nodes")])
        ):
    ...
    def add_gauges(self, gauge_coords):
        ...
        for gauge in gauge_coords:
            ...
            if all(
                ...
                gauge_index = [
                    int(round((gauge[n] - self.lower[n]) / self.delta[n]))
                    for n in range(self.num_dim)
                ]
                gauge_file_name = (
                    "gauge" + "_".join(str(coord) for coord in gauge) + ".txt"
                )
...
class Dimension(object):
    ...
    def nodes_with_ghost(self, num_ghost):
        ...
        nodes = self.nodes
        pre = np.linspace(
            self.lower - num_ghost * self.delta, self.lower - self.delta, num_ghost
        )
        post = np.linspace(
            self.upper + self.delta, self.upper + num_ghost * self.delta, num_ghost
        )
    ...
    def __init__(
        ...
        if isinstance(lower, str):
            raise Exception(
                "Passing dimension name as first argument is deprecated. \
