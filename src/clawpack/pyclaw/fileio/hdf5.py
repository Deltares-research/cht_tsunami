    ...
    if use_h5py:
        with h5py.File(filename, "w") as f:
            ...
            for state in solution.states:
                ...
                subgroup.attrs["dimensions"] = [
                    name.encode("utf-8") for name in patch.get_dim_attribute("name")
                ]
                # Dimension properties
                for dim in patch.dimensions:
                    for attr in ["num_cells", "lower", "delta", "upper", "units"]:
                        if hasattr(dim, attr):
...
def read(solution, frame, path="./", file_prefix="claw", read_aux=True, options={}):
    r"""
    ...
    if use_h5py:
        with h5py.File(filename, "r") as f:
            for patch in f.values():
                ...
                state = pyclaw.state.State(
                    pyclaw_patch, patch.attrs["num_eqn"], patch.attrs["num_aux"]
                )
                state.t = patch.attrs["t"]
                state.q = (
                    patch["q"][:].ravel(order="F").reshape(state.q.shape, order="F")
                )
                ...
                if read_aux and patch.get("aux", None) is not None:
                    state.aux = (
                        patch["aux"][:]
                        .ravel(order="F")
                        .reshape(state.aux.shape, order="F")
                    )
