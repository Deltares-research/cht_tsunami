...
def write(
    ...
    sol_dict = {
        "t": solution.t,
        "num_eqn": solution.num_eqn,
        "nstates": len(solution.states),
        "num_aux": solution.num_aux,
        "num_dim": solution.domain.num_dim,
        "write_aux": write_aux,
...
def read(solution, frame, path="./", file_prefix="fort", read_aux=False, options={}):
    r"""
    ...
    [t, num_eqn, nstates, num_aux, num_dim, num_ghost, file_format] = read_t(
        frame, path, file_prefix
    )
        ...
        with open(fname, "r") as f:
            for state in solution.states:
                ...
                for i in range(len(patch.dimensions)):
                    assert (
                        patch.dimensions[i].num_cells
                        == aux_patch.dimensions[i].num_cells
                    ), (
                        "Dimension %s's num_cells in aux file header did not match patch no %s."
                        % (patch.dimensions[i].name, patch.patch_index)
                    )
                    assert np.abs(
                        patch.dimensions[i].lower - aux_patch.dimensions[i].lower
                    ) <= ABS_TOL + REL_TOL * np.abs(patch.dimensions[i].lower), (
                        "Value of lower in aux file does not match."
                    )
                    assert np.abs(
                        patch.dimensions[i].delta - aux_patch.dimensions[i].delta
                    ) <= ABS_TOL + REL_TOL * np.abs(patch.dimensions[i].delta), (
