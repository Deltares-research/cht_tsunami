...
def select_by_flooding(
    ...
    set_next_i, set_next_j = where(
        logical_and(pt_chosen == 1, pt_chosen_neighbor_sum < 0)
    )

    nfront = len(set_next_i)
    if verbose:
        print(
            "Initially: %i cells out of %i cells on frontier" % (nfront, prod(Z.shape))
        )

    if max_iters is None:
        max_iters = prod(Ztopo.shape)  # upper bound, should need much fewer

    print("Selecting points with Z1 = %g, Z2 = %g, max_iters=%i" % (Z1, Z2, max_iters))
    ...
    for k in range(max_iters):
        ...
        if verbose == 2:
            print(
                "Iteration %3i: there are %7i points on frontier" % (k, len(set_next_i))
            )
            ...
            for ii, jj in [(i - 1, j), (i + 1, j), (i, j - 1), (i, j + 1)]:
                if (ii >= 0) and (ii <= maxi) and (jj >= 0) and (jj <= maxj):
                    if increasing:
                        ...
                        cond1 = Ztopo[ii, jj] > Z2
                    if (pt_chosen[ii, jj] < 0) and cond1 and (not mask[ii, jj]):
                        pt_chosen[ii, jj] = 1
    ...
    print(
        "Done after %i iterations with %i points chosen" % (iter_count, pt_chosen.sum())
    )
