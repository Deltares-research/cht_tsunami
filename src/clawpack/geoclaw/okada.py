...
def builddynamicdeffile(
    okadaparamfile, faultparamfile, outfile, t0=0.0, tend=1.0, nt=2
):
...
def getokadaparams(infile):
    ...
    for key in keylist:
        if key not in okadaparams:
            print(
                (
                    "ERROR: parameters for okada fault not fully specified in %s"
                    % (infile)
                )
            )
            exit
...
def getfaultparams(infile):
    ...
    if ("dx" in faultgridparams) & ("dy" in faultgridparams):
        faultgridparams["xupper"] = faultgridparams["xlower"] + faultgridparams[
            "dx"
        ] * (faultgridparams["mx"] - 1)
        faultgridparams["yupper"] = faultgridparams["ylower"] + faultgridparams[
            "dy"
        ] * (faultgridparams["my"] - 1)
    elif ("xupper" in faultgridparams) & ("yupper" in faultgridparams):
        faultgridparams["dx"] = (
            faultgridparams["xupper"] - faultgridparams["xlower"]
        ) / (faultgridparams["mx"] - 1)
        faultgridparams["dy"] = (
            faultgridparams["yupper"] - faultgridparams["ylower"]
        ) / (faultgridparams["my"] - 1)
    else:
    ...
    for key in keylist:
        if key not in faultgridparams:
            print(
                (
                    "ERROR: parameters for fault grid not fully specified in %s"
                    % (infile)
                )
            )
            exit
...
def dip_slip(x1, x2, x3, y1, y2, dp, dd):
    ...
    a5 = (
        ...
        * 2
        / cs
        * arctan((y2 * (xx + q * cs) + xx * (r + xx) * sn) / y1 / (r + xx) / cs)
    )
    f = -(d_bar * q / r / (r + y1) + sn * arctan(y1 * y2 / q / r) - a5 * sn * cs) / (
        2.0 * 3.14159
    )
