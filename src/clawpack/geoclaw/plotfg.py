...
from clawpack.visclaw import colormaps, geoplot, plotpages
...
class ClawPlotFGData(ClawData):
    ...
    def get_frame(self, frameno):
        ...
        grid.xcenter = grid.x[:-1] + grid.dx / 2.0
        grid.ycenter = grid.y[:-1] + grid.dy / 2.0
    ...
    def plotfg(self, frameno):
        ...
        def add_contours():
            ...
            contour(
                grid.xcenter, grid.ycenter, solution.B, [0.0], colors="k", linewidths=2
            )

        # t_str = "%10.3f" % solution.t
        tmin = solution.t / 60.0
        ...
        if solution.ncols > 5:
            ...
            etamax2 = where(solution.B < 0, 1.0, etamax)
        ...
        if self.inundated_show:
            ...
            if ma.count(inundated) != 0:
                ...
                if self.inundated_add_colorbar:
                    colorbar(shrink=0.5)
            add_contours()
            # Add red contour of maximum eta
            # contour(xcenter,ycenter,etamax2,[drytol],colors='r',linewidths=2)
    ...
    def fg2html(self, framenos="all"):
        ...
        ppd.html_index_fname = "_PlotIndex_FixedGrid%s.html" % str(self.fgno).zfill(2)
