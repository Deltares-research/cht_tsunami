...
class FGmaxGrid(object):
    ...
    def __init__(self):
        ...
        self.n23 = None
        self.tstart_max = 0.0
        self.tend_max = 1.0e10  # when to stop monitoring max values
        self.dt_check = 10.0  # target time (sec) increment between updating
        # max values
        self.min_level_check = None  # which levels to monitor max on
        self.interp_method = 0  # 0 for pw const, 1 for bilinear
        self.arrival_tol = 1.0e-2  # tolerance for flagging arrival
    ...
    def read_fgmax_grids_data(self, fgno, data_file="fgmax_grids.data"):
        ...
        if fgmax_input is None:
            raise ValueError("fgmax grid fgno = %i not found in %s" % (fgno, data_file))
    ...
    def write_to_fgmax_data(self, fid):
        ...
        if point_style not in [0, 1, 2, 3, 4]:
            raise NotImplementedError(
                "make_fgmax not implemented for point_style %i" % point_style
            )
        ...
        if point_style == 2:
            ...
            # print("Created file ", self.input_file_name)
            print(
                "   specifying fixed grid with shape %i by %i, with  %i points"
                % (nx, ny, npts)
            )
            ...
            print("       %i by %i, with  %i points" % (self.n12, self.n23, npts))
    ...
    def read_output(self, fgno=None, outdir=None, verbose=True, indexing="ij"):
        r"""
        ...
        fname = os.path.join(self.outdir, "fgmax%s.txt" % str(self.fgno).zfill(4))
        ...
        if ncols not in cols_expected:
            raise IOError(
                "*** Unexpected number of columns %s in file %s" % (ncols, fname)
            )
        ...
        if point_style in [0, 1, 4]:
            ...
            raise NotImplementedError(
                "Not implemented for point_style %s" % point_style
            )
        ...
        level = numpy.reshape(
            d[:, ind_level].astype("int"), fg_shape, order=reshape_order
        )
        ...
        arrival_time = numpy.reshape(
            d[:, ind_arrival_time], fg_shape, order=reshape_order
        )
        ...
        if 0:
            ...
            if self.force_dry_init is not None:
                ...
                self.h_onshore = ma.masked_where(self.B0 < 0.0, self.h)
    ...
    def ps4_to_arrays(self, verbose=True):
        ...
        for attr in zarrays:
            ...
            if z_1d is None:
                if verbose:
                    print("not converting attribute %s == None" % attr)
            else:
                ...
                setattr(self, attr, Z)
                if verbose:
                    print("converted %s to 2d array" % attr)
    ...
    def interp_dz(self, dtopo_path, dtopo_type):
        ...
        x1d = dtopo.X[0, :]
        y1d = dtopo.Y[:, 0]
        dtopo_func = RegularGridInterpolator(
            (x1d, y1d),
            dtopo.dZ[-1, :, :].T,
            method="linear",
            bounds_error=False,
        ...
        print(
            "Over fgmax extent, min(dz) = %.2f m, max(dz) = %.2f m"
            % (dz.min(), dz.max())
        )
...
def adjust_fgmax_grid(
    ...
    if verbose:
        print("x:")
        print(
            "  moved %17.12f to %17.12f by %g"
            % (x1_desired, x1_new, abs(x1_desired - x1_new))
        )
        print(
            "  moved %17.12f to %17.12f by %g"
            % (x2_desired, x2_new, abs(x2_desired - x2_new))
        )
        print("y:")
        print(
            "  moved %17.12f to %17.12f by %g"
            % (y1_desired, y1_new, abs(y1_desired - y1_new))
        )
        print(
            "  moved %17.12f to %17.12f by %g"
            % (y2_desired, y2_new, abs(y2_desired - y2_new))
        )
        # print "  "
        # print "fg.nx = %g" % nx
        # print "fg.ny = %g" % ny
