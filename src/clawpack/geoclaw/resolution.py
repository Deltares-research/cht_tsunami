...
def calculate_resolution(
    ratios, base_resolutions=[0.25, 0.25], lat_long=True, latitude=24.0
):
    ...
    for level in range(1, num_levels):
        natural_resolutions[level, :] = (
            natural_resolutions[level - 1, :] / ratios[level - 1]
        )
    ...
    if lat_long:
        ...
        for level in range(num_levels):
            meter_resolutions[level, :] = topotools.dist_latlong2meters(
                natural_resolutions[level, 0], natural_resolutions[level, 1], latitude
            )
        ...
        if lat_long:
            level_string = " - ".join(
                (
                    level_string,
                    "(%sº,%sº)"
                    % (natural_resolutions[level, 0], natural_resolutions[level, 1]),
                )
            )
            level_string = " - ".join(
                (
                    level_string,
                    "(%s m,%s m)"
                    % (meter_resolutions[level, 0], meter_resolutions[level, 1]),
                )
            )
        else:
            level_string = " - ".join(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="resolutions", description="Compute the effective resolution of refinement"
    )
    parser.add_argument(
        "ratios",
        metavar="ratios",
    ...
    calculate_resolution(
        args.ratios,
        base_resolutions=args.base_resolutions,
