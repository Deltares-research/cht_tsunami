...
# Length
conversion_func["m"] = [lambda L: L, lambda L: L]
conversion_func["cm"] = [lambda L: L * 1e-2, lambda L: L / 1e-2]
conversion_func["km"] = [lambda L: L * 1e3, lambda L: L / 1e3]
conversion_func["miles"] = [lambda L: L * 1.60934e3, lambda L: L / 1.60934e3]
conversion_func["nmi"] = [lambda L: L * 1852.0, lambda L: L / 1852.0]
conversion_func["lat-long"] = [lambda L: L * LAT2METER, lambda L: L / LAT2METER]
units["length"] = collections.OrderedDict(
    {
        "m": "meters",
        "cm": "centimeters",
        "km": "kilometers",
...
conversion_func["hPa"] = [lambda P: P * 1e2, lambda P: P / 1e2]
conversion_func["KPa"] = [lambda P: P * 1e3, lambda P: P / 1e3]
conversion_func["MPa"] = [lambda P: P * 1e6, lambda P: P / 1e6]
conversion_func["GPa"] = [lambda P: P * 1.0e9, lambda P: P / 1.0e9]
conversion_func["mbar"] = [lambda P: P * 1e2, lambda P: P / 1e2]
conversion_func["dyne/cm^2"] = [lambda P: P * 0.1, lambda P: P / 0.1]
conversion_func["dyne/m^2"] = [lambda P: P * 1.0e-5, lambda P: P / 1.0e-5]
units["pressure"] = {
    "Pa": "pascals",
    "hPa": "hectopascals",
    "KPa": "kilopascals",
    ...
    "GPa": "gigapascals",
    "mbar": "millibar",
    "dyne/cm^2": "Dynes/cm^2",
    "dyne/m^2": "Dynes/m^2",
}

# Speeds
conversion_func["km/h"] = [lambda v: v * 1e3 / (60**2), lambda v: v * 60**2 / 1e3]
conversion_func["mph"] = [lambda v: v * 0.44704, lambda v: v / 0.44704]
conversion_func["m/s"] = [lambda v: v, lambda v: v]
conversion_func["knots"] = [lambda v: v * 0.51444444, lambda v: v / 0.51444444]
units["speed"] = {
    "m/s": "meters/second",
    "knots": "knots (nm / hour)",
...
conversion_func["dyne-cm"] = [lambda M: M * 1.0e-7, lambda M: M / 1.0e-7]
units["moment"] = {"N-m": "Newton-Meters", "dyne-cm": "Dynes-Centimeter"}

# Temperature
conversion_func["C"] = [lambda temp: temp, lambda temp: temp]
conversion_func["F"] = [
    lambda temp: (temp - 32.0) * 5.0 / 9.0,
    lambda temp: temp * 9.0 / 5.0 + 32.0,
]
conversion_func["K"] = [lambda temp: temp + 273.15, lambda temp: temp - 273.15]
...
def convert(value, old_units, new_units, verbose=False):
    ...
    if found_type is None:
        raise ValueError(
            "Units %s not found in list of " % str(old_units), "supported conversions."
        )

    if new_units not in units[found_type].keys():
        raise ValueError(
            "Units %s not found in list of " % str(new_units),
