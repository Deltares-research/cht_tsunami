...
#  Constants
# ==============================================================================
from clawpack.geoclaw.data import DEG2RAD, RAD2DEG, Rearth
...
def inv_haversine(d, x1, y1, y2, Rsphere=Rearth, units="degrees"):
    ...
    dsigma = d / Rsphere
    cos_dsigma = (numpy.cos(dsigma) - numpy.sin(y1) * numpy.sin(y2)) / (
        numpy.cos(y1) * numpy.cos(y2)
    )
...
def gctransect(x1, y1, x2, y2, npts, coords="W", units="degrees", Rearth=Rearth):
    ...
    """
    from numpy import arcsin, arctan2, array, cos, dot, linspace, pi, sin, sqrt, zeros
...
def fetch_noaa_tide_data(
    ...
    def fetch(product, expected_header, col_idx, col_types):
        ...
        if os.path.exists(cache_path):
            if verbose:
                print("Using cached {} data for station {}".format(product, station))
            return parse(cache_path, col_idx, col_types, header=True)

        # otherwise, retrieve data from NOAA and cache it
        if verbose:
            print("Fetching {} data from NOAA for station {}".format(product, station))
    ...
    def get_cache_path(product):
        ...
        dates = "{}_{}".format(
            begin_date.strftime(cache_date_fmt), end_date.strftime(cache_date_fmt)
        )
        filename = "{}_{}_{}".format(time_zone, datum, units)
        abs_cache_dir = os.path.abspath(cache_dir)
        return os.path.join(abs_cache_dir, product, str(station), dates, filename)
    ...
    try:
        date_time2, prediction = fetch(
            "predictions", "Date Time, Prediction", col_idx, col_types
        )
