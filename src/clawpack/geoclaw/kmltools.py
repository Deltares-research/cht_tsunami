...
def deg2dms(dy):
    ...
    dy_deg = floor(dy)
    dy_min = floor((dy - dy_deg) * 60.0)
    dy_sec = (dy - dy_deg - dy_min / 60.0) * 3600.0
...
def regions2kml(rundata=None, fname="regions.kml", verbose=True, combined=True):
    ...
    dx = (x2 - x1) / float(mx)
    dx_meters = dx * 111e3 * cos(pi * 0.5 * (y1 + y2) / 180.0)
    dy = (y2 - y1) / float(my)
    dy_meters = dy * 111e3
    if verbose:
    ...
    if amr_levels_max > num_ref_ratios + 1:
        raise IOError(
            "*** Too few refinement ratios specified for "
    ...
    for k, r in enumerate(refinement_ratios_y):
        ...
        dy_deg, dy_min, dy_sec = deg2dms(dy)
        levtext = (
            "Level %s resolution:  dy = %g deg, %g min, %g sec = %g meters  (refined by %i)\n"
    ...
    for rnum, region in enumerate(regions):
        ...
        if verbose:
            ...
            print(
                "           minlevel = %i,  maxlevel = %i" % (minlevel, maxlevel)
...
def line2kml(xy, fname="line.kml", name="line", color="00FFFF", width=3, verbose=True):
    """
...
def poly2kml(
    ...
    width=3,
    verbose=True,
    max_vertices_in_description=20,
):
    """
...
def kml_timespan(t1, t2, event_time=None, tz=None, tscale=1):
    ...
    if event_time == None:
        ...
        if tz is None:
            print(
                "===> Time zone offset not defined;  assuming zero offset. "
...
def fgmax2kml(rundata=None, fname="fgmax_grids.kml", verbose=True, combined=False):
    ...
    for fg in fgmax_grids:
        ...
        if fg.point_style == 2:
            ...
            print("fgmax2kml not yet implemented for point_style = %i" % fg.point_style)
...
def pcolorcells_for_kml(
    X, Y, Z, png_filename=None, dpc=2, max_inches=15.0, verbose=True, **kwargs
):
    """
    ...
    if verbose:
        print(
            "Using kml_dpi = %i,figure size %.6f by %.6f inches"
            % (kml_dpi, x_inches, y_inches)
        )
        print("Figure has %i by %i grid cells of uniform color" % (x_cells, y_cells))
...
def png2kml(
    ...
    kml_text = kml_header(fname) + "<name>%s</name>\n" % name + "<open>1</open>\n"
...
def kml_build_colorbar(
    ...
    cb1 = mpl.colorbar.ColorbarBase(
        ax1, cmap=cmap, norm=norm, extend=extend, orientation="vertical"
    )
...
def topo2kmz(
    ...
    cmap_water = colormaps.make_colormap({0.0: [0, 0, 1], 1.0: [0.8, 0.8, 1]})

    cmap_topo, norm_topo = colormaps.add_colormaps(
        (cmap_land, cmap_water), data_limits=(zlim[0], zlim[1]), data_break=sea_level
    )

    cmap_force_dry = colormaps.make_colormap(
        {0.0: [1.0, 0.7, 0.7], 1.0: [1.0, 0.7, 0.7]}
    )
