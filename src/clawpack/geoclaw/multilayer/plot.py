...
def eta(cd, layer):
    return extract_eta(cd.q[layer_index[layer], :, :], cd.q[eta_index + layer, :, :])
...
def eta_elevation(surface, DRY_TOL=10**-3):
    ...
    def eta_func(cd):
        ...
        return numpy.ma.masked_where(
            b < eta_init, eta, numpy.ma.masked_where(numpy.abs(h) < DRY_TOL, h)
        )
...
def water_u(cd, direction, layer):
    return extract_velocity(
        cd.q[layer_index[layer], :, :], cd.q[layer_index[layer] + direction + 1, :, :]
    )
...
def add_combined_profile_plot(plot_data, slice_value, direction="x", figno=120):
    ...
    plotfigure = plotdata.new_plotfigure(
        name="combined_surface_%s" % figno, figno=figno
    )
    ...
    def top_surf_afteraxes(cd):
        ...
        axes.plot(
            [multilayer_data.bathy_location, multilayer_data.bathy_location],
            top_surf_zoomed,
            "--k",
        )
        axes.set_ylabel("m")
...
def add_velocities_profile_plot(plot_data, slice_value, direction="x", figno=130):
    ...
    # Velocities
    plotfigure = plotdata.new_plotfigure(
        name="combined_velocities_%s" % figno, figno=figno
    )
    plotfigure.show = True
