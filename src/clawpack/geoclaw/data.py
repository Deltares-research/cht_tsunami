...
class GeoClawData(clawpack.clawutil.data.ClawData):
    ...
    def write(self, data_source="setrun.py", out_file="geoclaw.data"):
        ...
        self.data_write("gravity", description="(gravitational acceleration m/s^2)")
        self.data_write("rho", description="(Density of water kg/m^3)")
        self.data_write("rho_air", description="(Density of air kg/m^3)")
        self.data_write(
            "ambient_pressure", description="(Nominal atmospheric pressure Pa)"
        )
        self.data_write("earth_radius", description="(Radius of the earth m)")
        self.data_write("coordinate_system", description="(1=meters, 2=lon-lat)")
        self.data_write(
            "sphere_source", description="(0=none, 1=only in mass eqn, 2=all)"
        )
...
class TopographyData(clawpack.clawutil.data.ClawData):
    ...
    def write(self, data_source="setrun.py", out_file="topo.data"):
        ...
        self.open_data_file(out_file, data_source)
        self.data_write(
            name="topo_missing", description="replace no_data_value in topofile"
        )
        self.data_write(
            name="test_topography", description="(Type topography specification)"
        )
        if self.test_topography == 0:
            ...
            for tfile in self.topofiles:
                ...
                fname = os.path.abspath(
                    os.path.join(os.path.dirname(out_file), tfile[-1])
                )
                self._out_file.write("\n'%s' \n " % fname)
                self._out_file.write("%3i   # topo_type\n" % tfile[0])
        elif self.test_topography == 1:
            self.data_write(
                name="topo_location", description="(Bathymetry jump location)"
            )
            self.data_write(
                name="topo_left", description="(Depth to left of bathy_location)"
            )
            self.data_write(
                name="topo_right", description="(Depth to right of bathy_location)"
            )
            ...
            raise NotImplementedError(
                "Test topography type %s has not been"
...
class FixedGridData(clawpack.clawutil.data.ClawData):
    ...
    def write(self, data_source="setrun.py", out_file="fixed_grids.data"):
        # Fixed grid settings
        msg = (
            "rundata.fixed_grid_data is deprecated starting in v5.9.0,"
    ...
    def write(self, data_source="setrun.py", out_file="fgout_grids.data"):
        ...
        for fg in self.fgout_grids:
            ...
            if fg.fgno in fgno_list:
                msg = (
                    "Trying to set fgout grid number to fgno = %i" % fg.fgno
...
class FGmaxData(clawpack.clawutil.data.ClawData):
    ...
    def write(self, data_source="setrun.py", out_file="fgmax_grids.data"):
        if len(self.fgmax_files) > 0:
            msg = "*** fgmax_files has been deprecated, " + "use fgmax_grids instead."
            raise ValueError(msg)
        ...
        for fg in self.fgmax_grids:
            ...
            if fg.xy_fname is not None:
                fg.xy_fname = os.path.abspath(
                    os.path.join(os.path.dirname(out_file), fg.xy_fname)
                )
            ...
            if fg.fgno in fgno_list:
                msg = (
                    "Trying to set fgmax grid number to fgno = %i" % fg.fgno
                    + "\n             but this fgno was already used"
    ...
    def read(self, path="fgmax_grids.data", force=False):
        ...
        if len(fig_numbers) != num_fgmax_grids:
            raise ValueError(
                "Number of FGMaxGrid numbers found does not ",
...
class DTopoData(clawpack.clawutil.data.ClawData):
    def __init__(self):
        ...
        self.add_attribute("dt_max_dtopo", 1.0e99)
    ...
    def read(self, path="dtopo.data", force=False):
        ...
        if len(self.dtopofiles) != num_dtopo_files:
            raise IOError(
                "The number of dtopo files specified does not equal ",
                "the number found.",
            )
...
class QinitData(clawpack.clawutil.data.ClawData):
    ...
    def write(self, data_source="setrun.py", out_file="qinit.data"):
        ...
        for force_dry in self.force_dry_list:
            ...
            # same directory that out_file comes from
            fname = os.path.abspath(
                os.path.join(os.path.dirname(out_file), force_dry.fname)
            )
            self._out_file.write("\n'%s' \n" % fname)
...
class SurgeData(clawpack.clawutil.data.ClawData):
    ...
    def write(self, out_file="surge.data", data_source="setrun.py"):
        ...
        self.data_write(
            "wind_index",
            value=self.wind_index + 1,
            description="(Index into aux array - fortran indexing)",
        )
        self.data_write(
            "pressure_index",
            value=self.pressure_index + 1,
            description="(Index into aux array - fortran indexing)",
        )
        self.data_write(
            "display_landfall_time", description="(Display time relative to landfall)"
        )
        self.data_write()
        ...
        if isinstance(self.wind_refine, bool):
            if not self.wind_refine:
                self.data_write(
                    "wind_refine", value=False, description="(Refinement ratios)"
                )
        ...
        if isinstance(self.R_refine, bool):
            if not self.R_refine:
                self.data_write(
                    "R_refine", value=False, description="(Refinement ratios)"
                )
        ...
        if type(self.storm_specification_type) is not int:
            if self.storm_specification_type in self.storm_spec_dict_mapping.keys():
                if self.storm_specification_type in self.storm_spec_not_implemented:
                    raise NotImplementedError(
                        "%s has not been implemented." % self.storm_specification_type
                    )

                else:
                    self.data_write(
                        "storm_specification_type",
                        self.storm_spec_dict_mapping[self.storm_specification_type],
                        description="(Storm specification)",
                    )
            else:
                raise ValueError(
                    "Unknown storm specification type %s"
                    % self.storm_specification_type
                )
        else:
            self.data_write(
                "storm_specification_type", description="(Storm specification)"
            )
...
class FrictionData(clawpack.clawutil.data.ClawData):
    ...
    def write(self, out_file="friction.data", data_source="setrun.py"):
        ...
        self.data_write(
            "variable_friction", description="(method for setting variable friction)"
        )
        self.data_write(
            "friction_index",
            value=self.friction_index + 1,
            description=("(Index into aux array ", "- fortran indexing)"),
        )
        self.data_write()
        if self.variable_friction:
            ...
            self.data_write(
                value=len(self.friction_regions),
            ...
            # File based friction
            self.data_write(
                value=len(self.friction_files), alt_name="num_friction_files"
            )
            for friction_file in self.friction_files:
                # if path is relative in setrun, assume it's relative to the
                # same directory that out_file comes from
                fname = os.path.abspath(
                    os.path.join(os.path.dirname(out_file), friction_file)
                )
...
class MultilayerData(clawpack.clawutil.data.ClawData):
    ...
    def __init__(self):
        ...
        self.add_attribute("wave_tolerance", [1.0e-1, 1.0e-1])
    ...
    def write(self, out_file="multilayer.data", datasource="setrun.py"):
        ...
        self.data_write(
            "wave_tolerance",
            description=(
                "(Tolerance of surface perturbation per",
                " layer, used for refinement criteria)",
            ),
        )
        self.data_write(
            "layer_index",
            value=self.layer_index + 1,
            description=("(Index into aux array -", " fortran indexing)"),
        )
        self.data_write(None)
        self.data_write("check_richardson", description="(Check Richardson number)")
        self.data_write(
            "richardson_tolerance", description="(Tolerance for Richardson number)"
        )
        self.data_write(
            "eigen_method", description="(Method for calculating eigenspace)"
        )
        self.data_write(
            "inundation_method",
            description=("(Method for calculating inundation ", "eigenspace)"),
        )
        self.close_data_file()
...
class BoussData(clawpack.clawutil.data.ClawData):
    ...
    def __init__(self):
        ...
        self.add_attribute("bouss_min_depth", 10.0)
        self.add_attribute("bouss_solver", 3)
        self.add_attribute("bouss_tstart", 0.0)

    def write(self, out_file="bouss.data", data_source="setrun.py"):

        self.open_data_file(out_file, data_source)
        self.data_write("bouss_equations", description="0=SWE, 1=MS, 2=SGN")
        self.data_write("bouss_min_level", description="coarsest level to apply bouss")
        self.data_write("bouss_max_level", description="finest level to apply bouss")
        self.data_write("bouss_min_depth", description="depth to switch to SWE")
...
class GridData1D(clawpack.clawutil.data.ClawData):
    ...
    def write(self, out_file="grid.data", data_source="setrun.py"):
        ...
        if self.grid_type == 2:
            ...
            # same directory that out_file comes from
            fname = os.path.abspath(
                os.path.join(os.path.dirname(out_file), self.fname_celledges)
            )
            self._out_file.write("\n'%s'   =: fname_celledges\n " % fname)

        self._out_file.write("\n%s   =: monitor_fgmax" % str(self.monitor_fgmax)[0])
        self._out_file.write("\n%s   =: monitor_runup" % str(self.monitor_runup)[0])
        self._out_file.write(
            "\n%s   =: monitor_total_zeta" % str(self.monitor_total_zeta)[0]
        )
        self.close_data_file()
...
class BoussData1D(clawpack.clawutil.data.ClawData):
    ...
    def __init__(self):
        ...
        self.add_attribute("bouss_equations", 2)
        self.add_attribute("bouss_min_depth", 20.0)
