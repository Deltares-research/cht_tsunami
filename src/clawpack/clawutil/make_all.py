...
def make_all(examples_dir=".", make_clean_first=False, env=None):
    ...
    for directory in dir_list:
        ...
        if make_clean_first:
            # Run 'make clean':
            job = subprocess.Popen(["make", "clean"], stdout=fout, stderr=ferr)
            return_code = job.wait()

        # Run 'make all':
        job = subprocess.Popen(["make", "all"], stdout=fout, stderr=ferr, env=my_env)
        return_code = job.wait()
...
def make_notebook_htmls(examples_dir=".", make_clean_first=False, env=None):
    ...
    for directory in nb_dir_list:
        ...
        job = subprocess.Popen(
            ["make", "notebook_htmls"], stdout=fout, stderr=ferr, env=my_env
        )
        return_code = job.wait()
        ...
        job = subprocess.Popen(
            ["make", "README.html"], stdout=fout, stderr=ferr, env=my_env
        )
        return_code = job.wait()
