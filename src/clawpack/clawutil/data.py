...
def get_remote_file(
    ...
    if not os.path.exists(unarchived_output_path) or force:
        if ask_user:
            ans = input(
                "  Ok to download topo file and save as %s?  \n"
        ...
        if tarfile.is_tarfile(output_path) and unpack:
            if verbose:
                print(
                    "Un-archiving %s to %s..." % (output_path, unarchived_output_path)
                )
            ...
            with zipfile.ZipFile(output_path, mode="r") as zip_file:
                ...
                unarchived_output_path = "".join((unarchived_output_path, extension))
            if verbose:
                print("Done un-archiving.")
...
class ClawData(object):
    ...
    def __setattr__(self, name, value):
        ...
        if (name not in self._attributes) and (name[0] != "_"):
    ...
    def write(self, out_dir=""):
        ...
        for data_object in self.data_list:
            ...
            if isinstance(data_object, amrclaw.GaugeData):
                data_object.write(
                    self.clawdata.num_eqn, self.clawdata.num_aux, out_file=fpath
                )
            else:
...
class ClawInputData(ClawData):
    ...
    def __init__(self, num_dim):
        ...
        if num_dim == 1:
            ...
            self.add_attribute("lower", [0.0, 0.0])
            self.add_attribute("upper", [1.0, 1.0])
            ...
            self.add_attribute("lower", [0.0, 0.0, 0.0])
            self.add_attribute("upper", [1.0, 1.0, 1.0])
        ...
        self.add_attribute("checkpt_time_interval", 1000.0)
        self.add_attribute("checkpt_times", [1000.0])
    ...
    def write(self, out_file="claw.data", data_source="setrun.py"):
        ...
        if self.output_style == 1:
            ...
            if len(self.output_times) == 0:
                raise AttributeError(
                    "*** output_style==2 requires nonempty list" + " of output times"
                )
            ...
            raise AttributeError(
                "*** Unrecognized output_style: %s" % self.output_style
            )
        ...
        if self.num_dim == 1:
            ...
            if self.num_dim == 2:
                if self.transverse_waves in [0, "none"]:
                    ...
                    raise AttributeError(
                        "Unrecognized transverse_waves: %s" % self.transverse_waves
                    )
            else:  # 3D
                if self.transverse_waves in [0, "none"]:
                    self.transverse_waves = 0
                ...
                if self.transverse_waves not in [0, 10, 11, 20, 21, 22]:
                    raise AttributeError(
                        "Unrecognized transverse_waves: %s" % self.transverse_waves
                    )
            ...
            if self.dimensional_split in [0, "unsplit"]:
                ...
                if self.num_dim == 3:
                    raise AttributeError(
                        "Strang dimensional splitting not supported in 3D"
                    )
                else:
                    self.dimensional_split = 2
            else:
                raise AttributeError(
                    "Unrecognized dimensional_split: %s" % self.dimensional_split
                )
        ...
        if self.source_split in [0, "none"]:
            ...
            raise AttributeError("Unrecognized source_split: %s" % self.source_split)
        ...
        for i in range(len(self.limiter)):
            if self.limiter[i] in [0, "none"]:
                self.limiter[i] = 0
            elif self.limiter[i] in [1, "minmod"]:
                self.limiter[i] = 1
            elif self.limiter[i] in [2, "superbee"]:
                self.limiter[i] = 2
            elif self.limiter[i] in [3, "vanleer"]:
                self.limiter[i] = 3
            elif self.limiter[i] in [4, "mc"]:
                self.limiter[i] = 4
            elif self.limiter[i] in [5, "bw"]:
                self.limiter[i] = 5
            else:
                raise AttributeError("Unrecognized limiter: %s" % self.limiter[i])
        ...
        if len(self.bc_lower) != self.num_dim:
            raise AttributeError(
                "Incorrect number of lower BC codes (expected %d, got %d)"
                % (self.num_dim, len(self.bc_lower))
            )
        for i in range(self.num_dim):
            if self.bc_lower[i] in [0, "user"]:
                self.bc_lower[i] = 0
            elif self.bc_lower[i] in [1, "extrap"]:
                self.bc_lower[i] = 1
            elif self.bc_lower[i] in [2, "periodic"]:
                self.bc_lower[i] = 2
            elif self.bc_lower[i] in [3, "wall"]:
                self.bc_lower[i] = 3
            else:
                raise AttributeError("Unrecognized bc_lower: %s" % self.bc_lower[i])
        ...
        if len(self.bc_upper) != self.num_dim:
            raise AttributeError(
                "Incorrect number of upper BC codes (expected %d, got %d)"
                % (self.num_dim, len(self.bc_upper))
            )
        for i in range(self.num_dim):
            if self.bc_upper[i] in [0, "user"]:
                self.bc_upper[i] = 0
            elif self.bc_upper[i] in [1, "extrap"]:
                self.bc_upper[i] = 1
            elif self.bc_upper[i] in [2, "periodic"]:
                self.bc_upper[i] = 2
            elif self.bc_upper[i] in [3, "wall"]:
                self.bc_upper[i] = 3
            else:
                raise AttributeError("Unrecognized bc_upper: %s" % self.bc_upper[i])
        ...
        if self.checkpt_style in [-2, 2]:
            ...
            self.data_write(
                name="", value=num_checkpt_times, alt_name="num_checkpt_times"
            )
            ...
            raise AttributeError(
                "*** Unrecognized checkpt_style: %s" % self.checkpt_style
            )
