...
# ============================================================================
def write_env_files(
    claw_path, verbose=True, outfile_base="setenv", shell_type="both", **kargs
):
    # Find projects
    available_projects = {}
    print("Found the following Clawpack projects:")
    for repo in git_repos:
        if repo in list(kargs.keys()):
            if os.path.exists(kargs[repo]):
                ...
                raise Exception(
                    "Project %s not found at custom path %s" % (repo, kargs[repo])
                )
        else:
    ...
    for project in available_projects.keys():
        missing_projects = check_repos_dependencies(
            project, list(available_projects.keys())
        )
        if missing_projects is not None:
            error_msg = (
                "The project %s depends on the following missing projects:"
                % project_name
            )
    ...
    if "clawutil" in available_projects:
        ...
        write_environment_variable(
            csh_file, bash_file, "CLAWUTIL", available_projects["clawutil"]
        )
        python_path = ":".join(
            (os.path.join(available_projects["clawutil"], "src", "python"), python_path)
        )

    if "classic" in available_projects:
        print("  CLASSICCLAW = %s" % available_projects["classic"])
        write_environment_variable(
            csh_file, bash_file, "CLASSICCLAW", available_projects["classic"]
        )

    if "amrclaw" in available_projects:
        print("  AMRCLAW = %s" % available_projects["amrclaw"])
        write_environment_variable(
            csh_file, bash_file, "AMRCLAW", available_projects["amrclaw"]
        )

    if "geoclaw" in available_projects:
        python_path = ":".join(
            (os.path.join(available_projects["geoclaw"], "src", "python"), python_path)
        )
        print("  GEOCLAW = %s" % available_projects["geoclaw"])
        write_environment_variable(
            csh_file, bash_file, "GEOCLAW", available_projects["geoclaw"]
        )

    if "pyclaw" in available_projects:
        python_path = ":".join(
            (os.path.join(available_projects["pyclaw"], "src"), python_path)
        )
        print("  PYCLAW = %s" % available_projects["pyclaw"])
        write_environment_variable(
            csh_file, bash_file, "PYCLAW", available_projects["pyclaw"]
        )

    if "clawapps" in available_projects:
        print("  CLAWAPPS = %s" % available_projects["clawapps"])
        write_environment_variable(
            csh_file, bash_file, "CLAWAPPS", available_projects["clawapps"]
        )
    ...
    if "riemann" in available_projects:
        python_path = ":".join(
            (os.path.join(available_projects["riemann"], "src", "python"), python_path)
        )
        print("  RIEMANN = %s" % available_projects["riemann"])
        write_environment_variable(
            csh_file, bash_file, "RIEMANN", available_projects["riemann"]
        )

    if "visclaw" in available_projects:
        python_path = ":".join(
            (os.path.join(available_projects["visclaw"], "src", "python"), python_path)
        )
        matlab_path = ":".join(
            (os.path.join(available_projects["visclaw"], "src", "matlab"), matlab_path)
        )
        print("  VISCLAW = %s" % available_projects["visclaw"])
        write_environment_variable(
            csh_file, bash_file, "VISCLAW", available_projects["visclaw"]
        )

    if "shaprclaw" in available_projects:
        python_path = ":".join(
            (
                os.path.join(available_projects["sharpclaw"], "src", "python"),
                python_path,
            )
        )
        print("  SHARPCLAW = %s" % available_projects["sharpclaw"])
        write_environment_variable(
            csh_file, bash_file, "SHARPCLAW", available_projects["sharpclaw"]
        )
    ...
    if "clawpack-4.x" in available_projects:
        ...
        write_environment_variable(
            csh_file, bash_file, "CLAW_4", available_projects["clawpack-4.x"]
        )
...
if __name__ == "__main__":
    ...
    try:
        try:
            long_options = ["help", "output=", "verbose", "shell=", "claw="]
