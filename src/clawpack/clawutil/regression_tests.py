...
def test_subdirs(compare_dir=None, examples_dir=".", verbose=True, relocatable=False):
    ...
    print("    ", examples_dir)
    print(
        "Will compare to _plots in corresponding subdirectories of: \n   %s"
        % compare_dir
    )
    if 0:
        ans = input("Ok? ")
    ...
    for test_subdir in dir_list:
        ...
        os.chdir(subdir)
        try:
            regression_ok = imagediff.imagediff_dir(
                test_plots,
                compare_plots,
            ...
            print(
                "*** Error running imagediff with directories \n  %s\n  %s\n"
