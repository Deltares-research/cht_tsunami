    ...
    *runTest* and *tearDown* methods which implement various parts of the
    ...
    def get_remote_file(self, url, **kwargs):
        ...
        if self.temp_path is None:
            raise ValueError(
                "Temporary data directory has not been ",
                "set yet.  Try calling super before attempting ",
                "to get remote files.",
            )

        output_path = clawpack.clawutil.data.get_remote_file(
            url, output_dir=self.temp_path, **kwargs
        )
    ...
    def build_executable(self, executable_name="xclaw"):
        ...
        shutil.move(os.path.join(self.test_path, self.executable_name), self.temp_path)
    ...
    def run_code(self):
        ...
        runclaw.runclaw(
            xclawcmd=os.path.join(self.temp_path, self.executable_name),
    ...
    def check_frame(
        ...
        regression_data_file = os.path.join(
            self.test_path, "regression_data", file_name
        )
        if save:
    ...
    def check_gauges(
        self, save=False, gauge_id=1, indices=[0], rtol=1e-14, atol=1e-8, tolerance=None
    ):
        ...
        if save:
            ...
            shutil.copy(
                os.path.join(self.temp_path, gauge_file_name), regression_data_path
            )
            claw_git_status.make_git_status_file(outdir=regression_data_path)

        regression_gauge = gauges.GaugeSolution(gauge_id, path=regression_data_path)
        ...
        try:
            ...
            err_msg = "\n".join(
                (e.args[0], "Gauge Match Failed for gauge = %s" % gauge_id)
            )
            err_msg = "\n".join((err_msg, "  failures in fields:"))
            failure_indices = []
            for n in indices:
                if ~numpy.allclose(
                    gauge.q[n, :], regression_gauge.q[n, :], rtol=rtol, atol=atol
                ):
        ...
        *success* is *False*, copies the contents of the tempory directory to
        ...
        if not self.success:
            output_dir = os.path.join(
                os.getcwd(), "%s_output" % self.__class__.__name__
            )
