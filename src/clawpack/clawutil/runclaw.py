...
def runclaw(
    ...
    if os.path.isdir(outdir) & (not overwrite):
        ...
        if verbose:
            ...
            if restart:
                print(
                    "==> runclaw: Copying directory to:      ",
                    os.path.split(outdir_backup)[1],
                )
            else:
                print(
                    "==> runclaw: Moving directory to:      ",
                    os.path.split(outdir_backup)[1],
                )
            time.sleep(1)
    ...
    if isinstance(xclawout, str):
        xclawout = open(xclawout, "w", encoding="utf-8", buffering=1)
    if isinstance(xclawerr, str):
        xclawerr = open(xclawerr, "w", encoding="utf-8", buffering=1)
    try:
        proc = subprocess.check_call(
            cmd_split, cwd=outdir, stdout=xclawout, stderr=xclawerr
        )

    except subprocess.CalledProcessError as cpe:
        exe_error_str = "\n\n*** FORTRAN EXE FAILED ***\n"
        raise ClawExeError(
            exe_error_str, cpe.returncode, cpe.cmd, output=cpe.output, stderr=cpe.stderr
        )
