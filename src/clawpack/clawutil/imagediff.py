...
def make_imagediff(fname1, fname2, fname3="", verbose=False):
    ...
    os.system(
        "convert %s %s -compose subtract -composite -threshold 0.001 -negate %s"
...
def imagediff_dir(
    ...
    for f in files:
        ...
        if not os.path.isfile(os.path.join(dir1, fhtml)):
            fhtml = f
    ...
    regression_ok = alltrue([f in f_equal for f in regression_test_files])
    ...
    try:
        try:
            opts, args = getopt.getopt(
                argv[1:], "hv", ["help", "verbose", "relocatable"]
            )
        ...
        if os.path.isfile(args[0]) and os.path.isfile(args[1]):
            ...
            if len(args) == 3:
                ...
                sys.exit(
                    imagediff_dir(
                        args[0], args[1], verbose=verbose, relocatable=relocatable
                    )
                )
        else:
