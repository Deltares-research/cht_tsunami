...
def convert_setrun(setrun_file="setrun.py", claw_pkg=None, ndim=None):
    ...
    if claw_pkg is None:
        raise ValueError("*** Could not determine claw_pkg from %s" % setrun_file)
    else:
        print("claw_pkg = ", claw_pkg)

    if ndim is None:
        raise ValueError("*** Could not determine ndim from %s" % setrun_file)
    else:
        print("ndim = ", ndim)
    ...
    if ndim >= 2:
        bc_list = bc_list + ["ylower", "yupper"]
    if ndim == 3:
        bc_list = bc_list + ["zlower", "zupper"]
    for b in bc_list:
    ...
    if claw_pkg in ["amrclaw", "geoclaw"]:
        ...
        mapping["flag_richardson"] = str(c.tol > 0.0)
        mapping["flag_richardson_tol"] = abs(c.tol)
        mapping["flag2refine"] = str(c.tolsp > 0.0)
    ...
    if claw_pkg == "classic" and ndim == 1:
        template = open(clawutil + "/conversion/setrun_template_classic_1d.py").read()
        newtext = template.format(**mapping)

    elif claw_pkg == "classic" and ndim == 2:
        template = open(clawutil + "/conversion/setrun_template_classic_2d.py").read()
        newtext = template.format(**mapping)

    elif claw_pkg == "classic" and ndim == 3:
        template = open(clawutil + "/conversion/setrun_template_classic_3d.py").read()
        newtext = template.format(**mapping)

    elif claw_pkg == "geoclaw" and ndim == 2:
        template = open(
            clawutil + "/conversion/setrun_template_geoclaw_2d_shallow.py"
        ).read()
        newtext = template.format(**mapping)
        ...
        raise ValueError(
            ...
            + "for claw_pkg = %s, ndim = %s" % (claw_pkg, ndim)
        )
    ...
    for line in setrun_text:
        if "new_UserData" in line:
            if line.strip()[0] != "#":
                ...
                print(line)
                print("*** User data lines from %s have NOT been copied " % setrun_file)
...
def copy_Makefile(claw_pkg, ndim):
    if claw_pkg == "classic" and ndim == 1:
        try:
            ...
            os.system("cp %s/conversion/Makefile_classic_1d Makefile" % clawutil)
        except:
            raise Exception("*** Error copying Makefile")
    elif claw_pkg == "classic" and ndim == 2:
        try:
            os.system("mv Makefile Makefile_4.x")
            os.system("cp %s/conversion/Makefile_classic_2d Makefile" % clawutil)
        except:
            raise Exception("*** Error copying Makefile")
    elif claw_pkg == "classic" and ndim == 3:
        try:
            os.system("mv Makefile Makefile_4.x")
            os.system("cp %s/conversion/Makefile_classic_3d Makefile" % clawutil)
        except:
            raise Exception("*** Error copying Makefile")
    elif claw_pkg == "amrclaw" and ndim == 2:
        try:
            os.system("mv Makefile Makefile_4.x")
            os.system("cp %s/conversion/Makefile_amrclaw_2d Makefile" % clawutil)
        except:
            raise Exception("*** Error copying Makefile")
    elif claw_pkg == "geoclaw" and ndim == 2:
        try:
            os.system("mv Makefile Makefile_4.x")
            os.system(
                "cp %s/conversion/Makefile_geoclaw_2d_shallow Makefile" % clawutil
            )
        except:
            raise Exception("*** Error copying Makefile")
    else:
        raise ValueError(
            "*** copy_Makefile not yet implemented "
            + "for claw_pkg = %s, ndim = %s" % (claw_pkg, ndim)
        )
